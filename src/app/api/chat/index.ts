...
// 최초 대화 시작일을 조회하는 함수
export async function getEarliestChatDate(): Promise<string> {
  try {
    console.log('🔍 최초 대화 시작일 조회 중...');
    
    // 매우 넓은 범위로 데이터를 조회하여 최초 날짜 찾기
    const queryParams = new URLSearchParams({
      page: '0',
      pageSize: '1',
      startDate: '2020-01-01', // 충분히 이른 날짜
      endDate: new Date().toISOString().split('T')[0], // 오늘
      sortBy: 'timestamp',
      sortOrder: 'asc' // 오름차순으로 정렬하여 가장 이른 날짜 찾기
    });

    const fullUrl = `${API_BASE_URL}/chats?${queryParams}`;
    console.log('🔗 최초 날짜 조회 URL:', fullUrl);
    
    const response = await fetch(fullUrl);
    
    if (!response.ok) {
      const errorText = await response.text();
      console.error('❌ 최초 날짜 조회 API Error:', errorText);
      throw new Error(`Failed to fetch earliest chat date: ${response.status} ${response.statusText}`);
    }

    const data = await response.json();
    console.log('✅ 최초 날짜 조회 응답:', data);
    
    if (data.items && data.items.length > 0) {
      const earliestChat = data.items[0];
      const earliestDate = new Date(earliestChat.timestamp).toISOString().split('T')[0];
      console.log('📅 최초 대화 시작일:', earliestDate);
      return earliestDate;
    } else {
      // 데이터가 없으면 기본값으로 2024년 7월 1일 반환
      console.log('⚠️ 대화 데이터가 없어서 기본값 사용: 2024-07-01');
      return '2024-07-01';
    }
  } catch (error) {
    console.error('❌ 최초 대화 시작일 조회 실패:', error);
    // 에러 발생 시 기본값으로 2024년 7월 1일 반환
    return '2024-07-01';
  }
}